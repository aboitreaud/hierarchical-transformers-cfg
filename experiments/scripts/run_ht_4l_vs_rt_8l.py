...
Compare 4-layer HT vs 8-layer RT with matched parameter count
"""
...
def run_experiment(config_path):
    """Run the HT-4L vs RT-8L experiment"""
    ...
    for model_name, model_config in config['models'].items():
        ...
        if config.get('use_wandb', True):
            wandb.init(
                ...
                name=f"{model_name}_{model.get_num_params()/1e6:.1f}M_params",
                config={
                    'model': model_config,
                    'cfg': cfg_config,
        ...
        if config['logging'].get('save_checkpoints', False):
            checkpoint_dir = Path("experiments/results/ht_4l_vs_rt_8l/checkpoints")
    ...
    if len(results) == 2:
        ...
        acc_diff = results[models[1]]['final_accuracy'] - results[models[0]]['final_accuracy']
        param_diff = (results[models[1]]['parameters'] - results[models[0]]['parameters']) / 1e6
        print(f"\nParameter Efficiency Analysis:")
        print(f"Accuracy difference: {acc_diff:.2f}%")
        print(f"Parameter difference: {param_diff:.2f}M")
        if param_diff != 0:
            print(f"Accuracy per million parameters: {acc_diff/param_diff:.2f}%/M")
    
    return results


if __name__ == "__main__":
    config_path = Path(__file__).parent.parent / "configs" / "ht_4l_vs_rt_8l.yaml"
